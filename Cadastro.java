...
public class Cadastro extends Clientes implements Autentica {
    ...
    public void lista() {
        List<Clientes> MaisComprados = lista.stream().filter(c -> c.getCompras() >= 15).collect(Collectors.toList());
        System.out.println("-----------------------------");
        MaisComprados.forEach(c -> System.out
                .println("Maiores compradores: " + c.getNome() + "| Quantidade de compras: " + c.getCompras()));

        List<Clientes> MenosComprados = lista.stream().filter(c -> c.getCompras() <= 14).collect(Collectors.toList());
